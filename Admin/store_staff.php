...
if (isset($_GET['type']) && $_GET['type'] == 'delete') {
  ...
  if ($check > 0) {
    ...
    $isAlreadyDelete = 1;
    // echo "staff was already deleted or does not exist";
  }
}
  ...
  if ($check > 0) {
    ...
    if ($_FILES['image']['name'] != '') {
      if ($_FILES['image']['type'] != 'image/png' && $_FILES['image']['type'] != 'image/jpg' && $_FILES['image']['type'] != 'image/jpeg') {
        ...
        $stmt = $con->prepare("INSERT INTO staff (s_name, s_uname, s_dob, s_gen, s_addr, s_email, s_phn, s_pass, s_join_date, s_salary, s_qualif, s_role, s_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'medicine_store_staff', '$img')");
      ...
      $stmt = $con->prepare("INSERT INTO staff (s_name, s_uname, s_dob, s_gen, s_addr, s_email, s_phn, s_pass, s_join_date, s_salary, s_qualif, s_role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'medicine_store_staff')");
      $stmt->bind_param("sssssssssis", $s_name, $s_uname, $s_dob, $s_gen, $s_addr, $s_email, $s_phn, $s_pass, $s_join_date, $s_salary, $s_qualif);
      $stmt->execute();
      $stmt->close();
      $isAdd = 1;
...
$res = mysqli_query($con, "select * from staff where staff.s_role='medicine_store_staff' order by staff.s_name;");
?>
<section class="admin-right-panel">
  <div class="admin-panel-alert">
    ...
    if ($isAlreadyExist == 1) {
      echo "<div class='bg-warning admin-alert-dismiss-container'>
                    <strong>Warning!</strong> Staff already exist
                    <button type='button' class='admin-alert-dismiss-btn'>&times;</button>
                </div>";
    }
    if ($isDelete == 1) {
      echo "<div class='bg-danger admin-alert-dismiss-container'>
                    <strong>Deleted!</strong> staff deleted successfully
                    <button type='button' class='admin-alert-dismiss-btn'>&times;</button>
                </div>";
    }
    ...
    if ($isAlreadyDelete == 1) {
      echo "<div class='bg-warning admin-alert-dismiss-container'>
            <strong>Warning!</strong> staff was deleted already or does not exist
  ...
  <div class="admin-right-top-bar">
    ...
    <span class="admin-breadcump">
      ...
      <span> / </span>
      <a href="store_staff.php" class="right-top-link active">Store</a>
      <span> / </span>
    </span>
  </div>
  <div class="admin-staffs-main">
    <div class="admin-add-container p-2">
      <button class="btn btn-warning btn-lg btn-block add-btn-dropdown m-2">Add Staff</button>
      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="staff-ad-form" class="p-2 admin-add-dorpdown-form admin-add-dorpdown-form-hide" enctype="multipart/form-data">
        <div class="form-group m-2">
          <label for="s_name" class="form-control-label">Staff Name</label>
          <input type="text" id="s_name" name="s_name" placeholder="Enter Staff name" class="form-control" required>
        </div>
        ...
        <div class="form-group m-2">
          ...
          <input type="text" id="s_uname" name="s_uname" class="form-control" placeholder="username will auto generate after you enter staff name and email address, username will be used while login" readonly>
        </div>
        <div class="form-group m-2">
        ...
        <div class="form-group m-2">
          ...
          <p class="log-err" id="email_error"></p>
        </div>
        <div class="form-group m-2">
        ...
        <div class="form-group m-2">
          ...
          <input type="password" id="s_pass" name="s_pass" placeholder="Create Password" class="form-control" required>
        </div>
        <div class="row p-2">
          <div class="col-6">
        ...
        <tbody>
          ...
          while ($row = mysqli_fetch_assoc($res)) { ?>
            <tr>
              ...
              <td>
                <?php
                echo "<span class='btn btn-warning'><a href='manage_store_staff.php?type=edit&s_id=" . $row['s_id'] . "' class='manage-links'>Edit</a></span>&nbsp;";
